...
<script>
import { getArticleList, delArticle, recoveryArticle } from '@api/sys.login'
...
export default {
  ...
  methods: {
    ...
    recoveryArticleList (data) {
      recoveryArticle(data).then(res => {
        console.log(res)
        setTimeout(() => {
          this.getArticle()
        }, 1000)
    ...
    handleEdit (index, row) {
      ...
      this.recoveryArticleList(id)
    },
    handleEditAll (index, row) {
      let delId = []
      this.selectItem.forEach(res => {
        delId.push(res.id)
      })
      console.log(delId)
      this.recoveryArticleList(delId)
    },
